...
class TestTruncateResult:
    ...
    def test_large_string_data_truncated(self):
        ...
        assert "truncated" in str(truncated["data"])
    ...
    def test_large_list_data_truncated(self):
        result = {"status": "success", "data": list(range(1000)), "message": "ok"}
        truncated = truncate_result(result, max_chars=500)
        assert truncated["_truncated"] is True
...
class TestFormatActionHistory:
    ...
    def test_failed_action_shows_x(self):
    ...
    def test_truncates_to_max_chars(self):
        actions = [{"cycle": i, "tool": f"skill:action{i}", "result": {"status": "success", "message": "m" * 200}, "params": {"p": "v" * 100}} for i in range(20)]
        result = format_action_history(actions, max_total_chars=500)
        assert len(result) <= 550  # some slack for truncation message
...
class TestHelpers:
    ...
    def test_summarize_params_long_value(self):
        result = _summarize_params({"content": "x" * 100})
        assert "..." in result
        assert len(result) <= 60

    def test_summarize_data_string(self):
        result = _summarize_data("hello")
        assert '"hello"' in result

    def test_summarize_data_dict(self):
